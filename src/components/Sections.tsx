...
/* Fade + rise when the element enters the viewport (one-shot) */
function Reveal({
  children,
  delay = 0,
  className = "",
...
/* Word-by-word reveal driven by how far the element has scrolled.
   The whole line starts dim; words light up in sequence as the
   runway scrolls (progressive text highlight/reveal). */
function WordReveal({
  text,
  className = "",
  from = 0.15,
  ...
  const on = Math.round(p >= from ? Math.min(1, (p - from) / (to - from)) * words.length : 0);
...
/* ------------------------------------------------------------------ */
/* Background grid field (scrollytelling backdrop)                     */
/* ------------------------------------------------------------------ */
...
function GridField() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 z-0">
      ...
      <div className="grid-beam grid-beam-alt" />
      <div className="grid-walker" />
      <div className="grid-walker grid-walker-alt" />
      <div className="grid-walker grid-walker-slow" />
    </div>
  );
}
...
/* A tall scroll runway whose sticky child is pinned for the length
   of the runway — fullscreen scrollytelling. */
function Slide({
  id,
  children,
  className = "",
  ...
  return (
    ...
    >
      <div className="sticky top-0 flex min-h-svh items-center overflow-hidden px-6 py-24 md:px-14 md:py-28">
        <GridField />
        <SlideDecor />
...
function Hero({ t, lang }: { t: Copy; lang: Lang }) {
  ...
  return (
    <Slide id="top">
      ...
      <Reveal delay={100}>
        <h1 className="mx-auto mt-8 max-w-4xl text-center font-display text-[clamp(2.4rem,6vw,5.2rem)] leading-[0.98] tracking-[-0.02em]">
...
function Positioning({ t, lang }: { t: Copy; lang: Lang }) {
  ...
  return (
    <Slide id="positioning" className="h-[180vh]">
      <Reveal>
      ...
      <WordReveal
        text={p.body}
        className="mx-auto mt-12 max-w-3xl text-center font-serif text-[clamp(1.4rem,2.4vw,1.9rem)] leading-snug"
      />
...
function About({ t }: { t: Copy }) {
  const a = t.about;
  return (
    <Slide id="about" className="h-[200vh]">
      <Reveal>
      ...
      <WordReveal
        text={a.statement}
        className="mx-auto mt-10 max-w-5xl text-center font-display text-[clamp(2.2rem,5vw,4.6rem)] leading-[1.05] tracking-[-0.015em]"
      />
  ...
  return (
    <Slide id="build" className="h-[180vh]">
      ...
      <Reveal delay={100}>
        ...
        <WordReveal
          text={b.intro}
          className="mx-auto mt-6 max-w-2xl text-center font-serif text-lg leading-relaxed"
        />
      </Reveal>
...
function Projects({ t, lang }: { t: Copy; lang: Lang }) {
  ...
  return (
    <Slide id="projects" className="h-[220vh]">
      <Reveal>
      ...
      <div ref={activeRef} className="mt-12 border-b border-cream/10">
        {w.projects.map((proj, i) => {
          const a = Math.min(1, Math.max(0, (p - i / w.projects.length) * w.projects.length));
          ...
          return (
            <a
              ...
              className="group grid gap-3 border-t border-cream/10 py-6 transition-colors duration-300 hover:bg-raised/40 md:grid-cols-12 md:items-center md:gap-6 md:py-7"
            >
              <div className="md:col-span-4">
                <h3
                  className={`font-display text-2xl leading-tight transition-colors duration-300 md:text-3xl ${
                    active ? "text-flame" : "text-cream group-hover:text-flame"
                  }`}
                >
                  {proj.name}
                </h3>
...
function Contact({ t }: { t: Copy }) {
  const c = t.contact;
  return (
    <Slide id="contact" className="h-[160vh]">
      <Reveal>
      ...
      <Reveal delay={180}>
        <WordReveal
          text={c.body}
          className="mx-auto mt-8 max-w-2xl text-center font-serif text-lg leading-relaxed"
        />
      </Reveal>
